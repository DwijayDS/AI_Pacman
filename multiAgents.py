    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        for i in GhostPosition:
            ...
            if newScaredTimes[len(ghost_dist)-1] == 0:
                ...
                #print(newScaredTimes[len(ghost_dist)-1])
                if ghost_dist[-1] < 4:
                    ghost_to_run_towards += 1/ghost_dist[-1]
                #ghost_dist_weighted_active = 0
...
def betterEvaluationFunction(currentGameState):
    ...
    for i in Power_pallet:
        ...
        if pallet_dist[-1] <= 2 and pallet_dist[0]>0:
    ...
    Eval_func += (1*go_behind_pallet) + (1*eat_power_pallet)
    Eval_func += (1*ghost_to_run_towards)
    
    return Eval_func
    # Extra score if closer to food
