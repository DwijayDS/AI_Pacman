...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        food_dist = []
        food_dist_weighted = 0 
        # Distance from ghost
        ghost_dist = []
        ghost_dist_weighted = 0
        # time_parameter
        time_to_eat_ghost = 0
        
        
        #Calculating number of food items remaining
        food_rem = len(newFood.asList())
        ...
        for i in newFood.asList():
            ...
            food_dist_weighted += 1/food_dist[-1]
            #if food_dist[-1] > 20:
            #    print('FOOD FAR')
        ...
        for i in GhostPosition:
            ...
            if ghost_dist[-1] < 4:
                #print('Ghost Close')
                ghost_dist_weighted -= (5)*ghost_dist[-1]
            elif ghost_dist[-1] < 8:
                #print('Ghost Mid')
                ghost_dist_weighted -= (0.5)*ghost_dist[-1]
            else:
                #print('Ghost away')
                ghost_dist_weighted -= (0.05)*ghost_dist[-1]

        
        # Checking if we have eaten the power pallet
        # If newScaredTimes for any ghost is more than 0 then we have eaten the 
        # power pallet
        time_to_eat_ghost = min(newScaredTimes)
        # COST
        ...
        if time_to_eat_ghost > 0:
            ...
            eva_func = SGSS +food_dist_weighted + ghost_dist_weighted
        ...
        def value(state,depth,agent):
            ...
            if agent == self.index:
                ...
                for possible_action in state.getLegalActions(agent):
                    ...
                    #print(" agent:",agent," Present Eval:",Eval)
                    if Eval > v :
